...
export class ExcalidrawCompiler {
	...
	compile: TCompilerStep = (file) => async (text) => {
		...
		if (file.getFrontmatter()["excalidraw-plugin"] === "parsed") {
			const svg = await convertToHTMLSVG(file.file.path, excalidrawApi);
			...
			text = this.serializer.serializeToString(div);
		...
		for (const link of links) {
			...
			if (isEmbedded) {
				const svg = await convertToHTMLSVG(filePath, excalidrawApi);
				...
				text = text.replace(
					match,
					this.serializer.serializeToString(div),
				);
			} else {
				text = text.replace(
					match,
					this.serializer.serializeToString(linkEl),
				);
			}
		}
