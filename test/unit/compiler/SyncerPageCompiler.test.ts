...
function makeMockPublishFile(
	...
	return {
		...
		getFrontmatter: vi.fn().mockReturnValue(overrides.frontmatter ?? {}),
		getMetadata: vi.fn().mockReturnValue({}),
		getBlock: vi.fn().mockReturnValue(undefined),
		meta: {
			getCreatedAt: vi.fn().mockReturnValue(null),
			getUpdatedAt: vi.fn().mockReturnValue(null),
			getPublishedAt: vi.fn().mockReturnValue(null),
		},
...
describe("SyncerPageCompiler", () => {
	...
	describe("extractBlobLinks", () => {
		it("extracts transcluded image paths", async () => {
			...
			(mc.getFirstLinkpathDest as Mock).mockReturnValue({
		...
		it("skips embeds that do not resolve", async () => {
			...
			(mc.getFirstLinkpathDest as Mock).mockReturnValue(null);
		...
		it("extracts asset references from canvas nodes", async () => {
			...
			(file.getType as Mock).mockReturnValue("canvas");
