	...
	describe("listTemplateNames", () => {
		it("returns directory names from templates dir", async () => {
			...
			const call = requireValue(
				mockRepo.listDirectory.mock.calls[0],
				"Expected listDirectory call",
			);
			assert.strictEqual(call[0], "quartz/cli/templates");
		});
	...
	describe("readTemplate", () => {
		it("reads and parses a template config", async () => {
			...
			const plugin = requireValue(
				result.config.plugins[0],
				"Expected template plugin",
			);

			assert.strictEqual(result.name, "blog");

			assert.strictEqual(
				result.config.configuration.pageTitle,
				"Blog Template",
			);
			assert.strictEqual(plugin.order, 10);
		});
